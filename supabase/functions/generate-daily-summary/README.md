...
7. Calls the Anthropic Claude API (`claude-sonnet-4-6`) to generate a structured daily briefing.
8. Falls back to a templated summary if the Claude call fails — the function always returns a result.
...
|---|---|
| `ANTHROPIC_API_KEY` | Anthropic API key used to call `claude-sonnet-4-6`. |
