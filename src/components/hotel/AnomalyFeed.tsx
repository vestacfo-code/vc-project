  ...
  return (
    <Card className="h-full border border-vesta-navy/10 bg-white shadow-sm">
      <CardHeader className="pb-3">
        ...
        <p className="text-xs text-vesta-navy/65">Last 7 days — unresolved</p>
      </CardHeader>
      <CardContent className="px-4 pb-4">
        {isLoading ? (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="space-y-2 py-2">
                ...
                <Skeleton className="h-3 w-24 bg-vesta-mist/50" />
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-sm font-medium text-vesta-navy/90">All clear!</p>
          ...
          <div className="space-y-3 max-h-80 overflow-y-auto pr-1">
            {anomalies.map((anomaly) => {
              ...
              return (
                <div
                  key={anomaly.id}
                  className="rounded-lg border border-vesta-navy/10 bg-vesta-mist/80 p-3"
                >
                  <div className="flex items-start justify-between gap-2 mb-1.5">
                    ...
                    <span className="text-xs text-vesta-navy/65 shrink-0">
                      {format(new Date(anomaly.date + 'T00:00:00'), 'MMM d')}
                    </span>
                  </div>
                  <p className="mb-1 text-xs font-semibold text-vesta-navy">{anomaly.title}</p>
                  <p className="mb-1 text-[11px] font-medium capitalize text-vesta-navy/65">
                    {anomaly.metric.replace(/_/g, ' ')}
                  </p>
                  <p className="mb-2 text-xs leading-relaxed text-vesta-navy/80">{anomaly.description}</p>
                  <Button
                    size="sm"
                    variant="ghost"
                    className="h-6 px-2 text-xs text-vesta-navy/80 hover:bg-emerald-50 hover:text-emerald-800"
