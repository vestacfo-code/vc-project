  ...
  return (
    <div className="flex-1 p-6 pt-16 space-y-6 overflow-y-auto bg-zinc-50">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-semibold text-zinc-900">Product Search</h1>
      ...
      <div className="relative max-w-2xl mx-auto">
        ...
        <Input
          ...
          className="pl-12 h-12 text-base rounded-xl border-zinc-200 bg-white shadow-sm focus-visible:ring-zinc-300"
        />
      </div>
      ...
      {isLoading ? (
        ...
        <div className="max-w-2xl mx-auto space-y-1">
          {searchResults.length === 0 ? (
            <div className="text-center py-16">
              <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-zinc-100 mb-4">
                <Search className="w-6 h-6 text-zinc-400" />
              </div>
              <p className="text-sm font-medium text-zinc-600">No products found</p>
              <p className="text-xs text-zinc-400 mt-1">Try a different search term or upload new data</p>
            </div>
          ) : (
            <>
              <p className="text-xs text-zinc-500 mb-3">{searchResults.length} result{searchResults.length !== 1 ? 's' : ''}</p>
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      className="w-full flex items-center gap-4 p-3 rounded-lg bg-white border border-zinc-100 hover:border-zinc-200 hover:shadow-sm transition-all duration-150 text-left group"
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-mono text-zinc-400">{product.upc}</span>
                          {product.brand && (
                            <span className="text-xs text-zinc-500 bg-zinc-50 px-1.5 py-0.5 rounded">{product.brand}</span>
                          )}
                        </div>
                      ...
                      <div className="flex items-center gap-4 flex-shrink-0">
                        {avgPrice && (
                          <span className="text-sm font-semibold text-zinc-700">${avgPrice.toFixed(2)}</span>
                        )}
                        <span className="text-xs text-zinc-400">{supplierCount} supplier{supplierCount !== 1 ? 's' : ''}</span>
        ...
        <div>
          <p className="text-xs font-medium text-zinc-500 uppercase tracking-wide mb-3">Recent Products</p>
          {recentProducts.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-xs text-zinc-400 mt-1">Upload your catalog to get started</p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {recentProducts.map((product, i) => {
                ...
                return (
                  ...
                  >
                    <Card
                      className="bg-white border-zinc-100 hover:border-zinc-200 hover:shadow-md transition-all duration-200 cursor-pointer group"
                      onClick={() => setSelectedProduct(product)}
                    >
                      <CardContent className="p-4">
                        <p className="text-[11px] font-mono text-zinc-400">{product.upc}</p>
                        <p className="text-sm font-medium text-zinc-900 mt-1 truncate">
                        ...
                        <div className="flex items-center justify-between mt-3 pt-2 border-t border-zinc-50">
                          {avgPrice ? (
                            ...
                            <span className="text-xs text-zinc-300">No price</span>
                          )}
                          <span className="text-[11px] text-zinc-400">{supplierCount} supplier{supplierCount !== 1 ? 's' : ''}</span>
                        </div>
                      </CardContent>
                    </Card>
                  </motion.div>
