    ...
    <>
      <tr
        className={cn(
          ...
          product.status === 'neutral' && 'border-l-transparent hover:bg-vesta-mist/25'
        )}
      ...
      >
        ...
        {editMode && (
          <td className="px-3 py-2.5" data-checkbox>
            <Checkbox checked={isSelected} onCheckedChange={() => onToggleSelect?.()} className="border-vesta-navy/12 data-[state=checked]:border-vesta-navy data-[state=checked]:bg-vesta-navy data-[state=checked]:text-white" />
          </td>
        )}
        ...
        <td className="px-3 py-2.5">
          <div className="flex items-center gap-1.5">
            <button className="p-0.5 hover:bg-vesta-mist/40 rounded flex-shrink-0">
              {isExpanded ? <ChevronUp className="w-3.5 h-3.5 text-vesta-navy/65" /> : <ChevronDown className="w-3.5 h-3.5 text-vesta-navy/65" />}
            </button>
            <span className="font-mono text-xs text-vesta-navy/80 truncate" title={product.upc}>
              {product.upc}
            </span>
                ...
                <Tooltip>
                  <TooltipTrigger>
                    <span className={cn(
                      ...
                      product.daysSinceReport > 7 && product.daysSinceReport <= 30 && 'text-vesta-navy/65 bg-vesta-mist/25',
        ...
        <td className="px-3 py-2.5">
          <div className="flex items-center gap-1.5">
            ...
            <span className="max-w-[100px] truncate rounded bg-vesta-mist/40 px-1.5 py-0.5 text-xs font-medium text-vesta-navy/90" title={product.brand || 'Unknown'}>
              {product.brand || '—'}
            </span>
            {product.variance !== 0 && product.base_cost && product.base_cost > 0 && (
              <span className={cn(
                ...
                product.status === 'neutral' && 'text-vesta-navy/65'
              )}>
        ...
        {/* Description */}
        <td className="px-3 py-2.5">
          <p className="text-xs text-vesta-navy/65 truncate max-w-[120px]" title={product.description || ''}>
            {product.description || '—'}
          </p>
        </td>
        ...
        <td className="px-3 py-2.5 text-right">
          {product.cogs && product.cogs > 0 ? (
            <span className="text-xs text-vesta-navy/65 tabular-nums">${product.cogs.toFixed(2)}</span>
          ) : (
            <span className="text-xs text-vesta-navy/80">—</span>
          )}
        </td>
        ...
        <td className="px-3 py-2.5 text-right">
          {product.base_cost && product.base_cost > 0 ? (
            <span className={cn(
              ...
              product.variance <= 2 && product.variance >= -2 && 'text-vesta-navy/50',
          ...
          return (
            <td key={supplier.id} className="px-3 py-2.5 text-right">
              {price ? (
                ...
                <span className="text-xs text-vesta-navy/90">—</span>
              )}
            </td>
          );
        })}
        ...
        {isExpanded && (
          <tr>
            <td colSpan={100}>
              ...
              >
                <div className="border-t border-vesta-navy/8 bg-vesta-mist/25 px-6 py-4">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <span className="text-sm font-medium text-vesta-navy/80">Price History</span>
                      {product.avg3m !== null && product.avg3m !== undefined && (
                        <span className="ml-3 text-xs text-vesta-navy/65">
                          3M: <span className="font-medium text-vesta-navy/65">${product.avg3m.toFixed(2)}</span>
                          {product.avg6m !== null && product.avg6m !== undefined && (
                            <> · 6M: <span className="font-medium text-vesta-navy/65">${product.avg6m.toFixed(2)}</span></>
                          )}
                        </span>
                      )}
                    </div>
                    <Button
                      ...
                      className="sticky right-4 h-7 px-3 text-vesta-navy/80 bg-vesta-mist/40 border-vesta-navy/10 hover:bg-vesta-mist/50 hover:text-vesta-navy"
                    >
