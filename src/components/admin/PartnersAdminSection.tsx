  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-vesta-navy">Partner marketplace</h2>
          <p className="text-sm text-vesta-navy/80">Manage featured vendors and product lines shown on /partners and in the hotel app.</p>
        </div>
      ...
      {loading && !partners.length ? (
        <div className="flex justify-center py-12 text-vesta-navy/65">
          <Loader2 className="h-8 w-8 animate-spin" />
        </div>
      ) : (
        <Card className="border-vesta-navy/10 bg-white shadow-sm">
          <CardHeader>
            <CardTitle className="text-base text-vesta-navy">Partners</CardTitle>
          </CardHeader>
          <CardContent className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow className="border-vesta-navy/10 hover:bg-transparent">
                  <TableHead className="text-vesta-navy/80">Name</TableHead>
                  <TableHead className="text-vesta-navy/80">Slug</TableHead>
                  <TableHead className="text-vesta-navy/80">Category</TableHead>
                  <TableHead className="text-vesta-navy/80">Active</TableHead>
                  <TableHead className="text-right text-vesta-navy/80">Products</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {partners.map((p) => (
                  <TableRow key={p.id} className="border-vesta-navy/10">
                    <TableCell className="font-medium text-vesta-navy">{p.name}</TableCell>
                    <TableCell className="font-mono text-xs text-vesta-navy/80">{p.slug}</TableCell>
                    <TableCell className="text-vesta-navy/80">{p.category}</TableCell>
                    <TableCell>
                      <Switch
      ...
      <Card className="border-vesta-navy/10 bg-white shadow-sm">
        <CardHeader>
          <CardTitle className="text-base text-vesta-navy">Product lines</CardTitle>
        </CardHeader>
        <CardContent className="overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow className="border-vesta-navy/10 hover:bg-transparent">
                <TableHead className="text-vesta-navy/80">Partner</TableHead>
                <TableHead className="text-vesta-navy/80">Name</TableHead>
                <TableHead className="text-vesta-navy/80">URL</TableHead>
                <TableHead className="w-24 text-vesta-navy/80" />
              </TableRow>
            </TableHeader>
            <TableBody>
              {products.map((pr) => {
                ...
                return (
                  <TableRow key={pr.id} className="border-vesta-navy/10">
                    <TableCell className="text-sm text-vesta-navy/80">{pn}</TableCell>
                    <TableCell className="text-vesta-navy">{pr.name}</TableCell>
                    <TableCell className="max-w-[200px] truncate text-xs text-vesta-navy/65">
                      {pr.product_url ?? '—'}
                    </TableCell>
                    <TableCell>
      ...
      <Dialog open={partnerOpen} onOpenChange={setPartnerOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto border-vesta-navy/10 bg-white text-vesta-navy">
          <DialogHeader>
          ...
          <div className="grid gap-4 py-2">
            <div>
              ...
              <Input
                ...
                className="border-vesta-navy/10 bg-white"
              />
            </div>
            <div>
              <Label>Name</Label>
            ...
            <div>
              ...
              >
                <SelectTrigger className="border-vesta-navy/10 bg-white">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div>
              ...
              <Textarea
                ...
                className="min-h-[80px] border-vesta-navy/10 bg-white"
              />
            </div>
            <div>
      ...
      <Dialog open={productOpen} onOpenChange={setProductOpen}>
        <DialogContent className="border-vesta-navy/10 bg-white text-vesta-navy">
          <DialogHeader>
          ...
          <div className="grid gap-4 py-2">
            ...
            <div>
              ...
              <Textarea
                ...
                className="min-h-[72px] border-vesta-navy/10 bg-white"
              />
            </div>
            <div>
