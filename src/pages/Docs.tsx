...
const SidebarNavSection = ({ section, onLinkClick }: { section: NavSection; onLinkClick?: () => void }) => {
  ...
  return (
    <Collapsible open={isOpen} onOpenChange={setIsOpen}>
      <CollapsibleTrigger className="flex items-center justify-between w-full py-2 text-xs font-semibold uppercase tracking-wider text-slate-500 hover:text-slate-800 transition-colors">
        {section.title}
          ...
          {section.items.map((item) => (
            <li key={item.title}>
              <Link
                to={item.href}
                onClick={onLinkClick}
                className="group flex items-center gap-2 rounded-md px-2 py-1.5 text-sm text-slate-600 transition-colors hover:bg-slate-100 hover:text-slate-900"
              >
  ...
  return (
    <div className="min-h-screen bg-vesta-cream text-slate-900">
      ...
      <div className="flex max-w-[1600px] mx-auto">
        {/* Left Sidebar */}
        <aside className="sticky top-[57px] hidden h-[calc(100vh-57px)] w-60 border-r border-slate-200 bg-white lg:block">
        ...
        <main className="flex-1 min-w-0">
          <div className="max-w-3xl mx-auto px-6 md:px-12 py-12">
            ...
            <section id="overview" className="mb-16">
              ...
              <div className="p-6 bg-gradient-to-br from-[#7ba3e8]/10 to-transparent border border-[#7ba3e8]/20 rounded-xl mb-8">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="mb-1 font-semibold text-slate-900">New to Vesta?</h3>
            ...
            <section id="quickstart" className="mb-16">
              <h2 className="mb-6 text-2xl font-semibold text-slate-900">Quickstart</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {quickstartCards.map((card) => (
                  <Link
                    key={card.title}
                    to={card.href}
                    className="group rounded-xl border border-slate-200 bg-white p-5 transition-all hover:border-slate-300 hover:bg-slate-50"
                  >
                    <div className="flex items-start justify-between">
                      <div className="mb-3 flex items-center gap-3">
                        <span className="text-slate-500 transition-colors group-hover:text-[#7ba3e8]">
                          {card.icon}
                        </span>
                        <h3 className="font-medium text-slate-900">{card.title}</h3>
                      </div>
                      <ChevronRight className="h-4 w-4 text-slate-400 transition-all group-hover:translate-x-0.5 group-hover:text-slate-900" />
                    </div>
            ...
            <section id="capabilities" className="mb-16">
              <h2 className="mb-6 text-2xl font-semibold text-slate-900">What you can do</h2>
              <div className="space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="font-medium text-slate-900">Connect your accounting software</h4>
                    <p className="text-sm text-slate-600">QuickBooks, Xero, Wave, or Zoho — sync in minutes</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <div className="p-1.5 bg-emerald-500/20 rounded mt-0.5">
                  ...
                  <div>
                    <h4 className="font-medium text-slate-900">Upload financial spreadsheets</h4>
                    <p className="text-sm text-slate-600">CSV, Excel, and other common formats supported</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <div className="p-1.5 bg-emerald-500/20 rounded mt-0.5">
                  ...
                  <div>
                    <h4 className="font-medium text-slate-900">Ask natural language questions</h4>
                    <p className="text-sm text-slate-600">"What's my profit margin this quarter?" — get instant answers</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <div className="p-1.5 bg-emerald-500/20 rounded mt-0.5">
                  ...
                  <div>
                    <h4 className="font-medium text-slate-900">Generate automated reports</h4>
                    <p className="text-sm text-slate-600">Weekly summaries, cash flow forecasts, expense breakdowns</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <div className="p-1.5 bg-emerald-500/20 rounded mt-0.5">
                  ...
                  <div>
                    <h4 className="font-medium text-slate-900">Visualize trends and patterns</h4>
                    <p className="text-sm text-slate-600">Interactive charts and dashboards that update in real-time</p>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section id="next-steps" className="mb-16">
              <h2 className="mb-6 text-2xl font-semibold text-slate-900">Next steps</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Link
                  to="/docs/getting-started/setup"
                  className="group flex items-center gap-4 rounded-xl border border-slate-200 bg-white p-4 transition-all hover:border-slate-300"
                >
                  <div className="rounded-lg bg-slate-100 p-2 transition-colors group-hover:bg-slate-200">
                    <Users className="h-5 w-5 text-slate-600" />
                  </div>
                  <div>
                    <h3 className="font-medium text-slate-900">Set up your account</h3>
                ...
                >
                  ...
                  <div>
                    <h3 className="font-medium text-slate-900">Take the quick tour</h3>
            ...
            <section className="rounded-xl border border-slate-200 bg-white p-6">
              <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
                ...
                <a
                  ...
                  className="flex items-center gap-2 rounded-lg border border-slate-200 px-4 py-2 text-sm text-slate-600 transition-all hover:bg-slate-50 hover:text-slate-900"
                >
                  Contact Support
